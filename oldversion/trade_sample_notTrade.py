...
#策略设置
strategy_set_dict = strategy_set_dict4
time_interval = '10m'
...
if __name__ == '__main__':
    ...
    limit_value = 2000000
    #计算当天单品种最大持仓的限制
    max_holdShares_df = computeMaxHold(portfolio_pos_df,limit_value,contract_multiplier_dict)
    ...
    #过风控
    portfolio_pos_df = riskControl(portfolio_pos_df,max_holdShares_df)
    ...
    elif (datetime.now().strftime('%H:%M') >= '11:30' and datetime.now().strftime('%H:%M') < '13:30'):
        run_time = now_time.replace(hour=13, minute=30, second=0, microsecond=0)
    ...
    if run_time == '':   #如果在交易时段
        correct_df,check_df,status_df,have_send_ding = checkAccountPos(main_engine,portfolio_pos_df,ignore_share_df,max_try_amount=5,needfix=True) #maxtry设置为2次，防止发生出现错误一直开仓的情况
        if have_send_ding == False: #如果持仓检查不需修复，则发送最新持仓到钉钉
            sendLastestPos(check_df)
    else:                 #如果非交易时段
        correct_df,check_df,status_df,have_send_ding = checkAccountPos(main_engine,portfolio_pos_df,ignore_share_df,max_try_amount=5,needfix=False)
        if correct_df.empty == False:
            ...
            correct_df,check_df,status_df,have_send_ding = checkAccountPos(main_engine,portfolio_pos_df,ignore_share_df,max_try_amount=5,needfix=True)
    ...
    while True:
        try:
            ...
            # run_time = datetime(2022,10,12,14,0)  #用于测试
            second0 = time.time()
            ...
            if run_time.strftime('%H:%M') in ['11:30','15:00']:
                ...
                sleep(0.0) #为防止整点获取的价格不稳定，做延迟1s的处理
            # 获取最新行情
            ...
            # pool2 = Pool(12)
            # #设置投资组合
            # portfolio_list =  list(portfolio_df_dict.keys())  
            # #运行多进程
            # # new_dict = multiprocessing.Manager().dict(portfolio_df_dict)
            # compute_list = [(i,portfolio_df_dict[i]) for i in portfolio_list]
            # result_list = pool2.imap_unordered(multi_computeSignal, compute_list)
            # second2 = time.time()
            # print('计算最新信号完成')  
            # print('用时%0.02fs'%(second2-second1))
            # second1 = time.time()
            # for result in result_list:
            #     # print(list(result[0].keys())[0] + ' 信号计算完成')
            #     # a = list(result[0].keys())[0]
            #     # b = list(result[0].values())[0] 
            #     # print(a)
            #     portfolio_df_dict[list(result[0].keys())[0]] = list(result[0].values())[0] 
            #     thisBarSignal_list += result[1]
            ...
            # #主进程阻塞等待子进程的退出 
            # # pool2.join()
            # # portfolio_df_dict = dict(new_dict)
            # # aa = portfolio_df_dict['rb99.SHFE'].tail(10)
            '''
            ...
            #             contractMulti = contract_multiplier_dict[re.sub("\d", "", i.split('.')[0])]
            #             capital_unit = strategy_set_dict[i][s]['capital_unit']
            #             portfolio_df_dict[i],thisBarSignal  = use_strategyForTrade( data_df = portfolio_df_dict[i],
            #                                                                         strategyname = s,
            #                                                                         contractMulti = contractMulti,
            #                                                                         para = para,
            #                                                                         capital_unit = capital_unit)
            ...
            #                 thisBarSignal_list.append(i.split('.')[0] +'   '+ thisBarSignal)
            #         #策略头寸汇总
            #         portfolio_df_dict[i]= countNewPos(portfolio_df_dict[i],strategy_set_dict[i],capital_unit,contractMulti,method)
            #         # second2 = time.time()
            #         # print('用时%0.02fs'%(second2-second1))  
            #     except Exception as e:
            #         log(str(e))
            ...
            # 过风控
            portfolio_pos_df_new = riskControl(portfolio_pos_df_new,max_holdShares_df)
            print('当前最新策略持仓（分策略）%s'%datetime.now().strftime('%Y%m%d %H:%M:%S'))
            print(portfolio_pos_df_new)
            log(portfolio_pos_df_new)
            # 根据最新持仓计算交易信号
            trade_df,change_df = computeTradeSingal(portfolio_pos_df,portfolio_pos_df_new)
            # 过滤掉忽略的合约
            ...
            if trade_df.empty == False:
                ...
                sleep(10)  #防止持仓未更新
                pos_df = getAcoountPos(main_engine)
                trade_df,change_df = computeTradeSingal(pos_df,portfolio_pos_df_new) 
            ...
            '''
            sleep(5) #防止持仓未更新    
            correct_df,check_df,status_df,have_send_ding = checkAccountPos(main_engine,portfolio_pos_df_new,ignore_share_df,max_try_amount=5,needfix=True)
            # 替换portfolio_pos_df
            '''
