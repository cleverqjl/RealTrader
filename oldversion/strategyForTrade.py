...
def brandySignal(data_df,strategyname,contractMulti,para=[400,1.5,8],capital_unit=100):  #long,short指做多做空，如果不做则改为0,如果做空short改为-1
    # ===策略参数
    n = int(para[0])
    m = para[1]
    atrn = para[2]
    ...
    if (close > upper) and (close2 <= upper2) and (last_signal <= 0) :
        data_df.loc[data_df.index[-1],strategyname+'_signal']  = 1
...
# para=[200,400,1.2,8]
# capital_unit=100   
def whiskySignal(data_df,strategyname,contractMulti,para=[100,200, 1, 6],capital_unit=100):  #long,short指做多做空，如果不做则改为0,如果做空short改为-1
    # ===策略参数
    n = int(para[0])
    fltn = int(para[1])
    m = para[2]
...
def tequilaSignal(data_df,strategyname,contractMulti,para=[100,200,50,200,6],capital_unit=100): 
    n = para[0]
    m = para[1]
    z = para[2]
    eman = para[3]
